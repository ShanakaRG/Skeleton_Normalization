...
def pre_normalization(data, zaxis=[0, 1], xaxis=[8, 4]):
    print('Activate orientation normalization')
    '''
    ...
    #                     rest = len(person) - i_f
    #                     # print('rest' , len(person) )
    #                     # print('gfguhuhi=========',len(person) , i_f)
    ...
    for i_s, skeleton in enumerate(tqdm(s)):
        ...
        main_body_center = skeleton[0][0, 1:2, :].copy()    # Shape (T, 1, C)
    ...
    for i_s, skeleton in enumerate(tqdm(s)):
        ...
        axis = np.cross(joint_top - joint_bottom, [0, 1, 0])
        angle = angle_between(joint_top - joint_bottom, [0, 1, 0])
